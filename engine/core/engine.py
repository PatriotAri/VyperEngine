...
from engine.ecs.system_base import FixedSystem, UpdateSystem, RenderSystem
from engine.ecs.read_only_world import ReadOnlyWorld
...
class Engine:
    ...
    def _tick(self) -> None:
        ...
        if self.window.should_close() or self.input_state.quit_requested:
            self.stop()
            return
        #Update clock (variable timestep)
        self.clock.tick()
